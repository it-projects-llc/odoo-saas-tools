...
4. Configure **Main Database**:
   * open Settings/Configuration/SaaS Portal Settings
     ...
     * click Apply (do it even if you don't make changes)

5. Register Server Database in Main Database
   * open SaaS/SaaS/Servers - click [Create]
   * set Database Name, e.g. **s1.odoo.local**
   * fix autogenerated Client ID to actual one (you can find it at Server Database: Settings/System Parameters -> database.uuid
   * click [Save]
...
6. Create Plan
   ...
   * set SaaS Server
   * set Template DB (via "Create and Edit"), e.g. **t1.odoo.local**
   * click [Save]
