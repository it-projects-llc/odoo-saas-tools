...
class res_users(models.Model):
    ...
    @api.model
    def _auth_oauth_rpc(self, endpoint, access_token, local_ip=None, local_port=None):
    ...
    def _auth_oauth_validate(self, provider, access_token):
        ...
        validation = self._auth_oauth_rpc(p.validation_endpoint, access_token, local_ip=p.local_ip, local_port=p.local_port)
        if validation.get("error"):
            raise Exception(validation['error'])
        if p.data_endpoint:
            data = self._auth_oauth_rpc(p.data_endpoint, access_token, local_ip=p.local_ip, local_port=p.local_port)
