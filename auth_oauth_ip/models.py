...
class res_users(models.Model):
    ...
    def _auth_oauth_rpc(self, cr, uid, endpoint, access_token, context=None, local_host=None, local_port=None):
    ...
    def _auth_oauth_validate(self, cr, uid, provider, access_token, context=None):
        ...
        validation = self._auth_oauth_rpc(cr, uid, p.validation_endpoint, access_token, context=context, local_host=p.local_host, local_port=p.local_port)
        if validation.get("error"):
            raise Exception(validation['error'])
        if p.data_endpoint:
            data = self._auth_oauth_rpc(cr, uid, p.data_endpoint, access_token, context=context, local_host=p.local_host, local_port=p.local_port)
