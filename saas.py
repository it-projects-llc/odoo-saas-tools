...
settings_group.add_argument('--odoo-longpolling-port', dest='longpolling_port', default=None, help='Port to run odoo temporarly')
...
xmlrpc_port = args.get('xmlrpc_port') or odoo_config.get('http_port') or '8069'
local_xmlrpc_port = args.get('local_xmlrpc_port') or odoo_config.get('http_port') or '8069'
...
# RPC Tools
# ----------------------------------------------------------
def rpc_auth(dbname, admin_username='admin', admin_password='admin', host='localhost', port=xmlrpc_port):
    main_url = 'http://%s:%s' % (host, port)
    if args.get('simulate'):
...
def rpc_add_server_to_portal(portal_db_name):
    ...
    #      * click [Save]
    auth = rpc_auth(portal_db_name, admin_password=args.get('admin_password'), host=args.get('local_portal_host'), port=local_xmlrpc_port)
