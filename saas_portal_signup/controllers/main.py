...
class AuthSignupHome(auth_signup.controllers.main.AuthSignupHome):
    ...
    def get_auth_signup_qcontext(self):
        ...
        if not qcontext.get('plans', False):
            ...
            qcontext['plans'] = sp.browse(request.cr, SUPERUSER_ID, plan_ids,
                                          context=context)
        if not qcontext.get('countries', False):
            orm_country = request.registry.get('res.country')
            country_ids = orm_country.search(request.cr, SUPERUSER_ID, [],
                                             context=context)
            countries = orm_country.browse(request.cr, SUPERUSER_ID,
                                           country_ids, context=context)
