...
class SaasPortalConfigWizard(models.TransientModel):
    ...
    def get_default_sftp_server(self, fields):
        sftp_server = self.env["ir.config_parameter"].sudo().get_param(
    ...
    def set_sftp_server(self):
        ...
        for record in self:
            config_parameters.set_param(
                "saas_server.sftp_server", record.sftp_server or '')

    # sftp_username
    @api.model
    def get_default_sftp_username(self, fields):
        sftp_username = self.env["ir.config_parameter"].sudo().get_param(
    ...
    def set_sftp_username(self):
        ...
        for record in self:
            config_parameters.set_param(
                "saas_server.sftp_username", record.sftp_username or '')
    ...
    def set_sftp_password(self):
        ...
        for record in self:
            config_parameters.set_param(
                "saas_server.sftp_password", record.sftp_password or '')

    # sftp_path
    @api.model
    def get_default_sftp_path(self, fields):
        sftp_path = self.env["ir.config_parameter"].sudo().get_param(
    ...
    def set_sftp_path(self):
        ...
        for record in self:
            config_parameters.set_param(
                "saas_server.sftp_path", record.sftp_path or '')
